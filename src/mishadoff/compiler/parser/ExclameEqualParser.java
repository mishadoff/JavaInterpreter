...
import mishadoff.compiler.tokens.ExclameEqualToken;
import mishadoff.compiler.tokens.Token;

public class ExclameEqualParser implements TokenParser{
	...
	public Token parseToken(String source, int fromIndex) {
		...
		Token token = new ExclameEqualToken(fromIndex, fromIndex+mText.length(), mText);
		return token;
	}
}
