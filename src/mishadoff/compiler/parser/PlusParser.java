...
public class PlusParser implements TokenParser{
	...
	public Token parseToken(String source, int fromIndex) {
		...
		Pattern p = Pattern.compile(".{"+fromIndex+"}(\\+{1}).*", Pattern.DOTALL);
		Matcher m = p.matcher(source);
		
		if (!m.matches()) return null;
		
		String mText = m.group(1);
		Token token = new PlusToken(fromIndex, fromIndex+mText.length(), mText);
		return token;
	}
}
