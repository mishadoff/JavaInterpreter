...
import mishadoff.compiler.tokens.LessEqualToken;
import mishadoff.compiler.tokens.Token;

public class LessEqualParser implements TokenParser{
	...
	public Token parseToken(String source, int fromIndex) {
		...
		Token token = new LessEqualToken(fromIndex, fromIndex+mText.length(), mText);
		return token;
	}
}
