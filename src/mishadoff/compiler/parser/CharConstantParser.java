...
public class CharConstantParser implements TokenParser{
	...
	public Token parseToken(String source, int fromIndex) {
		...
		Pattern p = Pattern.compile(".{"+fromIndex+"}\\b(\\'[0-9a-zA-Z]\\')\\b.*", Pattern.DOTALL);
		Matcher m = p.matcher(source);
		
		if (!m.matches()) return null;
		
		String mText = m.group(1);
		Token token = new CharConstantToken(fromIndex, fromIndex+mText.length(), mText);
		return token;
	}
}
