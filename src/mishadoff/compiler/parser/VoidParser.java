...
public class VoidParser implements TokenParser{
	...
	public Token parseToken(String source, int fromIndex) {
		...
		Pattern p = Pattern.compile(".{"+fromIndex+"}\\b(void)\\b.*",
				Pattern.DOTALL);		
		...
		Token token = new VoidToken(fromIndex, fromIndex+mText.length(), mText);
		return token;
	}
}
